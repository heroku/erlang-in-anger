...
Here are the possible values of keys, or at least commonly used ones:
...
\begin{description*}
	...
	\item[Location] \hfill
		\begin{description}
			\item[\expression{current\_function}] displays the current function as a tuple of the form \expression{\{Mod, Fun, Arity\}}.
			...
			\item[\expression{current\_stacktrace}] most verbose form of the two preceding options; displays the current stacktrace as a list of 'current locations'.
	...
	\item[Memory] \hfill
		\begin{description}
			...
			\item[\expression{min\_heap\_size}] Returns the smallest heap size a process can have according to the current configuration.
			
			\item[\expression{min\_bin\_vheap\_size}] Each Erlang process has to track reference-counted binaries \footnote{see Section \ref{sec:binaries}} to know when to trigger a garbage collection that may help reduce global memory usage. This is done in a virtual heap for binaries. This attribute refers to the minimal size this heap can have.
		\end{description}
	\item[Workload] \hfill
		\begin{description}
